...
function formatSchedule(schedule) {
  ...
  return schedule
    .map((lesson) => {
      const meeting = lesson.meetingLink ? ` · Google Meet: ${lesson.meetingLink}` : "";
...
function meetingBlock(booking) {
  ...
  return [
    ...
    "Class note: We use Google Meet for lessons. Open the link in your browser at class time.",
  ].filter(Boolean);
}
...
export function buildAdminBookingEmail(booking) {
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;color:#171a24;line-height:1.55">
      <h1 style="font-size:24px;margin:0 0 16px">New Mandrix booking</h1>
      ${htmlRows}
    </div>
  `;
  return { text, html };
...
export function buildStudentConfirmationEmail(booking) {
  ...
  const lines = [
    "Your Mandrix schedule is confirmed",
    "",
    `Hi ${booking.fullName},`,
    "",
    "Thank you for submitting your Mandrix booking. This email confirms that Mandrix has received your booking details and generated your class schedule.",
    "Your WorldFirst payment reference is recorded for order matching.",
    "",
    `Course: ${booking.course}`,
    `Amount: $${booking.amount || ""}`,
    ...
    `Frequency: ${booking.frequencyLabel || booking.frequency}`,
    "",
    ...(hasMeeting ? meetingLines : ["Google Meet link: Jane will send your class link separately if the default class link has not been configured yet."]),
    "",
    "Auto lesson schedule:",
    schedule,
    "",
    ...
    hasMeeting
      ...
      : "Jane will confirm the Google Meet link by email if a class link has not been configured yet.",
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;color:#171a24;line-height:1.55">
      <h1 style="font-size:24px;margin:0 0 16px">Your Mandrix schedule is confirmed</h1>
...
export async function sendBookingNotifications(booking) {
  ...
  let adminResult = await sendEmail({
      from,
      to,
      subject: `Mandrix booking: ${booking.fullName} · $${booking.amount || ""}`,
  ...
  if (!adminResult.ok && fallbackRecipient) {
    const fallbackResult = await sendEmail({
      ...
      subject: `[Fallback] Mandrix booking: ${booking.fullName} · $${booking.amount || ""}`,
  ...
  const studentResult = await sendEmail({
    from,
    to: booking.email,
    subject: "Your Mandrix schedule is confirmed",
